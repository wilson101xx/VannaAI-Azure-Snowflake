...
class AzureAISearch_VectorStore(VannaBase):
    ...
    Args:
        config (dict): Configuration dictionary. Defaults to {}. You must provide an API key in the config.
            ...
            - dimensions (int, optional): Dimensions of the embeddings. Defaults to 384 which corresponds to the dimensions of BAAI/bge-small-en-v1.5. will be changing but for now its fine, probs wanna use a larger one
            - fastembed_model (str, optional): Fastembed model to use. Defaults to "BAAI/bge-small-en-v1.5".  WE will be changing this but for the state of testing this is fine
